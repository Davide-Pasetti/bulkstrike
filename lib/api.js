...
export async function requestSamplesBulk({
  ...
  shippingAddress = null, quantityL = null, quantitaIndicativa = null, agentZones = null,
  ...
  // Mappa { supplier_company_id: zone_id }: la scelta dell'agente e' per
  // fornitore, l'invio invece e' cumulativo.
  if (agentZones && Object.keys(agentZones).length) payload.agent_zones = agentZones;
...
export async function requestSupplierContactBulk({ targetCompanyIds, productId, requestType, message = null, quantitaIndicativa = null, agentZones = null }) {
  const payload = {
  ...
  // Mappa { target_company_id: zone_id }: la scelta dell'agente è per fornitore.
  if (agentZones && Object.keys(agentZones).length) payload.agent_zones = agentZones;
...
/* ----- Agenti di vendita -----
 * Il legame agente-fornitore-zona serve a instradare i contatti. La provvigione
 * è un riferimento teorico fra fornitore e agente: BulkStrike non la incassa,
 * non la paga e non la traccia come pagamento. */

// Pannello fornitore: i propri legami, compresi quelli da confermare.
export async function myAgentZones() {
  const { data, error } = await supabase.rpc("my_agent_zones");
  if (error) throw error;
  return data || [];
}
// Il fornitore dichiara un proprio agente: nasce già confermato, lo dice lui.
export async function supplierAddAgent({ fullName, email, phone, country, region, commissionRate }) {
  const { data, error } = await supabase.rpc("supplier_add_agent", {
    payload: {
      full_name: fullName, email, phone: phone || null,
      country: country || null, region: region || null,
      commission_rate_indicative: commissionRate === "" || commissionRate == null ? null : commissionRate,
    },
  });
  if (error) throw error;
  return data;
}
// azione: 'conferma' | 'rifiuta' | 'disattiva'
export async function supplierSetAgentZone(zoneId, azione, rate = null) {
  const { data, error } = await supabase.rpc("supplier_set_agent_zone", {
    p_zone: zoneId, p_azione: azione, p_rate: rate,
  });
  if (error) throw error;
  return data;
}
// Cosa può sapere il buyer: nome e zona degli agenti confermati, mai la provvigione.
export async function agentiDiZona(supplierCompanyId, region = null, country = null) {
  const { data, error } = await supabase.rpc("agenti_di_zona", {
    p_supplier: supplierCompanyId, p_region: region, p_country: country,
  });
  if (error) return [];   // l'assenza di agenti non deve mai bloccare una richiesta
  return data || [];
}
// Ricerca fornitori per la pagina pubblica degli agenti: solo dati già
// visibili nella directory (ragione sociale, paese, città).
export async function cercaFornitoriPubblici(q, limit = 15) {
  const { data, error } = await supabase.rpc("cerca_fornitori_pubblici", { p_q: q, p_limit: limit });
  if (error) return [];
  return data || [];
}
// Pagina pubblica /agenti/registrati: l'agente si autocandida, il legame resta
// sospeso finché il fornitore non conferma.
export async function agentSelfRegister({ fullName, email, phone, suppliers }) {
  const { data, error } = await supabase.rpc("agent_self_register", {
    payload: { full_name: fullName, email, phone: phone || null, suppliers },
  });
  if (error) throw error;
  return data;
}
export async function adminListAgents() {
  ...
  if (error) throw error;
  return data || { agenti: [], provvigioni: [] };
}
export async function adminSetAgentZone(zoneId, status) {
  const { data, error } = await supabase.rpc("admin_set_agent_zone", { p_zone: zoneId, p_status: status });
  if (error) throw error;
  return data;
}
export async function adminAssignCommission(ledgerId, agentId) {
  const { data, error } = await supabase.rpc("admin_assign_commission", { p_ledger: ledgerId, p_agent: agentId });
  if (error) throw error;
  return data;
}
export const AGENT_ERRORS = {
  SOLO_FORNITORI: "Solo un profilo fornitore può dichiarare i propri agenti.",
  EMAIL_NON_VALIDA: "Indirizzo email non valido.",
  NOME_MANCANTE: "Indica il nome dell'agente.",
  PROVVIGIONE_NON_VALIDA: "La provvigione deve essere una percentuale fra 0 e 100.",
  NESSUN_FORNITORE_INDICATO: "Indica almeno un'azienda che rappresenti.",
  TROPPI_FORNITORI: "Puoi indicare al massimo 20 aziende per volta.",
  LIMITE_24H_RAGGIUNTO: "Hai già inviato molte richieste nelle ultime 24 ore: riprova domani.",
  AGENTE_NON_COLLEGATO_AL_FORNITORE: "Quell'agente non ha un legame confermato con questo fornitore.",
  NOT_ALLOWED: "Non puoi modificare questo collegamento.",
};
export function agentErrorMessage(e) {
  const raw = String(e?.message || e || "");
  for (const k of Object.keys(AGENT_ERRORS)) if (raw.includes(k)) return AGENT_ERRORS[k];
  return "Operazione non riuscita. Riprova.";
}
...
// Pubblica a mano nella conversazione una mail che il relay ha trattenuto
// perché il mittente non corrispondeva ai domini noti del fornitore. Il
// messaggio nasce contrassegnato come "mittente non verificato": è il motivo
// per cui è servito un intervento umano, e chi legge deve saperlo.
export async function adminPublishInboxEmail(id) {
  const { data, error } = await supabase.rpc("admin_pubblica_mail_inbox", { p_id: id });
  if (error) throw error;
  return data;
}
