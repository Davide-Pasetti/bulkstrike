...
export async function checkoutCart(shippingAddress, shippingNotes) {
  const { data, error } = await supabase.rpc("checkout_cart", { p_shipping_address: shippingAddress, p_shipping_notes: shippingNotes || null });
  if (error) throw error;
  return data; // { orders: [uuid...], count, total_paid }
}
...
export async function previewCheckout(shippingAddress) {
  const { data, error } = await supabase.rpc("preview_checkout", { p_shipping_address: shippingAddress || null });
  if (error) throw error;
  return data; // { lines:[{product_id,supplier_company_id,goods_subtotal,shipping_amount,vat_amount}], goods_subtotal, shipping_amount, vat_amount, total }
}
