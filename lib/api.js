...
// Richiesta di campionatura CUMULATIVA a più fornitori in una sola chiamata.
// La RPC gestisce ogni fornitore in modo indipendente (non tutto-o-niente) e
// torna un array: [{ supplier_product_id, status:'created'|'error',
// sample_request_id?, error_message? }]. Il limite di 5 richieste/24h vale su
// tutta la piattaforma, quindi con molte selezioni alcune possono fallire.
export async function requestSamplesBulk({ supplierProductIds, quantityL, shippingAddress, message = null }) {
  const { data, error } = await supabase.rpc("request_samples_bulk", {
    payload: {
  ...
  if (error) throw error; // errori globali (NOT_AUTHENTICATED, QUANTITA_NON_VALIDA, ...)
  return data || [];
}
