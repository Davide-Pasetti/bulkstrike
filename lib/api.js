...
export async function getProduct(productId) {
  ...
  const { data: product, error: pErr } = await supabase
    .from("products")
    .select("id, canonical_name, cas_number, e_number, iupac_name, description, formula, pallet_kg, default_unit, regulatory_flags, merch_classes, auction_restricted_by_law")
    .eq("id", productId)
    .maybeSingle();
  if (pErr) throw pErr;
