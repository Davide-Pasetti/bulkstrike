...
export async function getProduct(productId) {
  ...
  const suppliers = (sp || []).map((row) => {
    ...
    return {
      ...
      // per non mostrare ai clienti claim di variante non ancora controllati.
...
export async function previewCheckout(shippingAddress, carrierSelections) {
  // Esistono due versioni di preview_checkout sul DB: una a 1 parametro (stima
  // rapida per paese+kg, usata dal Carrello prima che si scelga un corriere) e
  // una a 2 parametri (prezzo corriere reale, usata al passo Pagamento del
  // Checkout). Se carrierSelections non viene passato affatto (Carrello),
  // NON includiamo p_carrier_selections nella chiamata RPC: cosi Postgres
  // risolve alla versione a 1 parametro invece di forzare spedizione=0 per i
  // fornitori senza corriere ancora selezionato.
...
// (campo role per distinguerli). NB: esiste anche getMyOrders (select diretta,
...
export async function upsertCarrierRate(rate) {
  const { data, error } = await supabase.rpc("upsert_carrier_rate", {
    ...
    p_lead_time_days: rate.leadTimeDays ?? null, // giorni di consegna per QUESTA tariffa (zona/distanza), non più un valore unico del corriere
  });
  if (error) throw error;
  return data;
}
