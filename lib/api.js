...
export async function adminListProductsPoolMin() {
  ...
  if (error) throw error;
  return data || []; // [{ id, canonical_name, pallet_kg, min_pool_pallets }]
}
...
export async function fetchOrderQrObjectUrl(orderId) {
  ...
  let token = anon;
  try { const { data } = await supabase.auth.getSession(); if (data?.session?.access_token) token = data.session.access_token; } catch (e) {}
