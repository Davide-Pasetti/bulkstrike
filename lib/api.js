...
export async function getProduct(productId) {
  ...
  const { data: sp, error: spErr } = await supabase
    ...
    .select(`
      ...
      unit_label, unit_size_kg, units_per_pallet,
  ...
  const suppliers = (sp || []).map((row) => {
    ...
    return {
      ...
      unit_label: row.unit_label || "sacco",
      unit_size_kg: Number(row.unit_size_kg) || 25,
      units_per_pallet: Number(row.units_per_pallet) || 40,
      tiers,
      best_price,
    };
  });
