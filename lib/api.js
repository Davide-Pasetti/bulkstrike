...
export async function getProduct(productId) {
  ...
  if (supplierIds.length > 0) {
    const { data: pubs, error: pubErr } = await supabase
      ...
      .select("id, legal_name, country, rating, reviews_count, status")
      .in("id", supplierIds);
...
// Nomi azienda per id, dalla vista pubblica dei fornitori. Sotto RLS un'azienda
// legge solo la PROPRIA riga companies: per la controparte usiamo suppliers_public
// (copre i fornitori; i puri acquirenti possono non esserci → fallback nel chiamante).
async function _companyNamesByIds(ids) {
  const uniq = [...new Set((ids || []).filter(Boolean))];
  if (uniq.length === 0) return new Map();
  const { data } = await supabase.from("suppliers_public").select("id, legal_name").in("id", uniq);
  return new Map((data || []).map((c) => [c.id, c.legal_name]));
}
...
// Fornitore: richieste ricevute (per la propria azienda).
export async function getSupplierSampleRequests() {
  ...
  const { data, error } = await supabase
    ...
    .select(`
      id, supplier_product_id, buyer_company_id, quantity_l, shipping_address, message,
      status, decline_reason, tracking_note, created_at, responded_at, shipped_at,
      product:products ( canonical_name, default_unit )
    `)
    .eq("supplier_company_id", companyId)
    .order("created_at", { ascending: false });
  if (error) throw error;
  const names = await _companyNamesByIds((data || []).map((r) => r.buyer_company_id));
  return (data || []).map((r) => ({ ...r, counterpart_name: names.get(r.buyer_company_id) || null }));
}

// Compratore: le proprie richieste inviate.
export async function getBuyerSampleRequests() {
  const companyId = await getMyCompanyId();
  if (!companyId) return [];
  const { data, error } = await supabase
    .from("sample_requests")
    .select(`
      id, supplier_product_id, supplier_company_id, product_id, quantity_l, shipping_address, message,
      status, decline_reason, tracking_note, created_at, responded_at, shipped_at,
      product:products ( canonical_name, default_unit )
    `)
    ...
    .order("created_at", { ascending: false });
  if (error) throw error;
  const names = await _companyNamesByIds((data || []).map((r) => r.supplier_company_id));
  return (data || []).map((r) => ({ ...r, counterpart_name: names.get(r.supplier_company_id) || null }));
}
