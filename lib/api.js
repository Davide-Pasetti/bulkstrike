...
export async function getProduct(productId) {
  ...
  const { data: product, error: pErr } = await supabase
    .from("products")
    .select("id, canonical_name, cas_number, e_number, iupac_name, description, formula, pallet_kg, default_unit, regulatory_flags, merch_classes, auction_restricted_by_law, scheda_sicurezza_url, scheda_tecnica_url")
    .eq("id", productId)
    .maybeSingle();
  if (pErr) throw pErr;
...
// ancora 'verified'), quindi senza prezzo né acquisto in piattaforma. Solo campi
// pubblici — nessun contatto diretto. Lettura pubblica (anche da anon).
//
...
// getSupplierProfile() (solo status='verified'), questa funziona anche sui
// pending — è il caso d'uso della coda di verifica. Gate _is_admin() lato DB.
