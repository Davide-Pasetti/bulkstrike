...
// Istanza LAZY (stesso fix del client admin): istanziare a livello di modulo
// fa fallire la build quando STRIPE_SECRET_KEY non è tra le env di build
// ("Collecting page data" valuta i moduli delle route). Niente apiVersion
// esplicita: usa il default pinnato dalla SDK installata (il valore
// precedente '2025-06-30' non era un identificatore di versione valido).
let _stripe = null;
...
export async function releaseFunds(p) {
  ...
  const intent = await getStripe().paymentIntents.retrieve(p.paymentIntentId);
  ...
  const supplierTransfer = await getStripe().transfers.create({
  ...
  const carrierTransfer = await getStripe().transfers.create({
...
export async function refund(p) {
  if (p.reverseTransferId) {
    await getStripe().transfers.createReversal(p.reverseTransferId, {
      amount: p.amountCents,
    });
  }
...
export async function getOnboardingStatus(connectedAccountId) {
  const acct = await getStripe().accounts.retrieve(connectedAccountId);
 ...
 * (Unificata qui da lib/escrowAdapter.js, che era un duplicato del file:
 * questo modulo in lib/payments/ è l'adapter canonico.)
 *
