...
-- Il drop serve perché aggiungere parametri a un CREATE OR REPLACE creerebbe un
-- OVERLOAD, lasciando in giro anche la versione a 11 argomenti.
drop function if exists public.email_richiesta_render(text,text,text,text,text,text,text,boolean,text,text,text);
...
begin
  ...
  -- quantità al prodotto; senza, {quantita} sparisce del tutto.
  v_apertura := replace(
...
-- e i tre argomenti finali di email_richiesta_render.
